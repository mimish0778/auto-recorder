...
`index.html` は外部ライブラリを読み込まない自己完結ファイルです（ネットワークが不通でもレイアウトは崩れません）。

**GitHub Pages で公開する場合**

1. リポジトリ直下に `index.html` を置いて push する
2. リポジトリの Settings → Pages → Source で公開ブランチ（例：`main` / `root`）を指定する
...
- GitHub Pages は HTTPS なので問題なく動作します
- ローカルの `index.html` をダブルクリックして開いた場合（`file://`）は、**ブラウザによってはカメラが起動しません**。その場合は GitHub Pages 上の URL から開くか、ローカルサーバ（例：`python -m http.server`）経由で `http://localhost:8000` として開いてください

**プライバシー上の注意**

- 収録した動画は**アップロードされません**。処理はすべてブラウザ内で完結し、動画は端末のダウンロードフォルダに保存されます
- ただし**公開リポジトリに被験者の動画を絶対にコミットしない**でください。GitHub Pages で公開するのはアプリ（`index.html`）のみとし、収録データはリポジトリ外で管理してください
